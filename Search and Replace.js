...
// Note: Preserve the case of the first character in the original word when you are replacing it. For example if you mean to replace the 
// word Book with the word dog, it should be replaced as Dog

function myReplace(str, before, after) {
    let result = '';
    let word = before[0].match(/[A-Z]/) ? after[0].toUpperCase() + after.slice(1) : after[0].toLowerCase() + after.slice(1);

    return result = str.split(' ').map(item => item.replace(before, word)).join(' ');
}
